...
(defn assert-hsl-maksatushakemuspaatos1-teksti [teksti vuosi osuus-avustuksesta haettuavustus]
...
(defn assert-hsl-maksatushakemuspaatos2-teksti [teksti mh1-paatospvm mh1-myonnettyavustus]
  (fact "HSL 2. maksatushakemuspäätöksen sisällön tarkastaminen"
      teksti => (partial strx/substring? (str "Liikenne- ja viestintävirasto on " mh1-paatospvm " tehdyllä päätöksellä maksanut "
...
(fact "Avustushakemuksen päätöksen esikatselu - suuri kaupunkiseutu"
  (test-ctx
    (let [id (hc/add-hakemus! hsl-ah0-hakemus)]
      ...
      (pdf/assert-header "Valtionavustuspäätös" "<päätöspvm>" nil)

      (pdf/assert-footer "esikatselu"))))
...
(fact "1. maksatushakemuksen päätöksen esikatselu"
  (test-ctx
    (let [kausi (test/next-avattu-empty-hakemuskausi!)
      ...
      (assert-hsl-maksatushakemuspaatos1-teksti paatos-pdf (:vuosi kausi) "<osuus avustuksesta>" 0)
...
(fact "2. maksatushakemuksen päätöksen esikatselu"
  (test-ctx
    (let [kausi (test/next-avattu-empty-hakemuskausi!)
      ...
      (assert-hsl-maksatushakemuspaatos2-teksti paatos-pdf "<maksatuspäätös pvm>" "<maksettu avustus>")
      (pdf/assert-header "Valtionavustuspäätös" "<päätöspvm>" nil)
  ...
  (fact
    (test-ctx
      (let [id (hc/add-hakemus! hsl-ah0-hakemus)
        ...
        (find-paatos-pdf id) => (partial strx/substring? expected)

        (pdf/assert-footer "esikatselu")))))

(fact "Avustushakemuksen päätös - päätöksen lisätiedot"
      (test-ah0-paatos-selite nil "kokonaiskustannuksista. Hakija")
      (test-ah0-paatos-selite "" "kokonaiskustannuksista. Hakija")
      (test-ah0-paatos-selite " " "kokonaiskustannuksista. Hakija")
      (test-ah0-paatos-selite "FooBar" "kokonaiskustannuksista. FooBar Hakija")
...
(fact "Voimassaolevan päätöksen hakeminen"
  (test-ctx
    (let [id (hc/add-hakemus! hsl-ah0-hakemus)
      ...
      (pdf/assert-header "Valtionavustuspäätös" "testing")
      (assert-hsl-avustushakemuspaatos-teksti (find-paatos-pdf id) vuosi)
...
(fact "Maksatushakemuksen päätöksen esikatselu avustushakemuksen päätös on tehty"
  (test-ctx
    (let [kausi (test/next-avattu-empty-hakemuskausi!)
      ...
      (let [pdf (find-paatos-pdf id)]
        (assert-hsl-maksatushakemuspaatos-teksti pdf (:vuosi kausi) "1.1. - 30.6." 1 pdf/today 1)
        (assert-hsl-maksatushakemuspaatos1-teksti pdf (:vuosi kausi) 100 1)
        (pdf/assert-header "Valtionavustuspäätös" "<päätöspvm>" "testing")
        (pdf/assert-footer "esikatselu")))))
    ...
    (asha/with-asha
      (pdf/with-mock-pdf
        (let [id (hc/add-hakemus! hsl-ah0-hakemus)
          ...
          (let [teksti (find-paatos-pdf id)]
            (pdf/assert-header "Valtionavustuspäätös" "testing")
            (pdf/assert-footer hc/kasittelija-organisaatio-name)
            (assert-hsl-avustushakemuspaatos-teksti teksti vuosi)
            teksti => (partial strx/substring? "Päätöksentekijä Osastopäällikkö Päivi Päätöksentekijä")
            teksti => (partial strx/substring? "Esittelijä Joukkoliikenneasiantuntija Katri Käsittelijä")))))))
...
(fact "Maksatushakemuksessa (mh1) osuusavustuksesta on päättymätön murtoluku"
      (test-ctx
        (let [kausi (test/next-avattu-empty-hakemuskausi!)
          ...
          (let [pdf (find-paatos-pdf mh1)]
            (assert-hsl-maksatushakemuspaatos-teksti pdf (:vuosi kausi) "1.1. - 30.6." "16 000" pdf/today "33 000")
            (assert-hsl-maksatushakemuspaatos1-teksti pdf (:vuosi kausi) 48 "16 000")

            pdf => (partial strx/substring? "PSA:n mukaisen liikenteen hankinta 16 000 € (alv 0%)")
...
(fact "Maksatushakemuksessa (mh1) osuusavustuksesta on 50%"
      (test-ctx
        (let [kausi (test/next-avattu-empty-hakemuskausi!)
          ...
          (let [pdf (find-paatos-pdf mh1)]
            (assert-hsl-maksatushakemuspaatos-teksti pdf (:vuosi kausi) "1.1. - 30.6." 1 pdf/today 2)
            (assert-hsl-maksatushakemuspaatos1-teksti pdf (:vuosi kausi) 50 1)
            (pdf/assert-header "Valtionavustuspäätös" "<päätöspvm>" "testing")))))
...
(fact "Maksatushakemus (mh1) - avustusta myönnetty 0€"
      (test-ctx
        (let [kausi (test/next-avattu-empty-hakemuskausi!)
          ...
          (let [pdf (find-paatos-pdf mh1)]
            (assert-hsl-maksatushakemuspaatos-teksti pdf (:vuosi kausi) "1.1. - 30.6." 1 pdf/today 0)
            (assert-hsl-maksatushakemuspaatos1-teksti pdf (:vuosi kausi)"**" 1)
            (pdf/assert-header "Valtionavustuspäätös" "<päätöspvm>" "testing")))))
...
(fact "Maksatushakemus (mh1) - maksuun ei haettu vielä mitään"
      (test-ctx
        (let [kausi (test/next-avattu-empty-hakemuskausi!)
          ...
          (assert-hsl-maksatushakemuspaatos1-teksti (find-paatos-pdf mh1) (:vuosi kausi) 0 0)
          (pdf/assert-header "Valtionavustuspäätös" "<päätöspvm>" "testing"))))
...
(fact
  ...
  (test-ctx
    (let [kausi (test/next-avattu-empty-hakemuskausi!)
      ...
      (let [txt (find-paatos-pdf ah0)]
        txt => (partial strx/substring? "Paikallisliikenne 1 €")
        txt => (partial strx/substring? "FooBar."))
